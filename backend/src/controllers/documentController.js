...
async function downloadLatest(req, res, next) {
  try {
    ...
    let buffer;
    try {
      buffer = await storage.load(version.fileUrl);
    } catch {
      // Distinct from a 500: this means the DB record exists but the
      // underlying file genuinely isn't in storage — e.g. it was created
      // before Supabase Storage was configured, was deleted directly from
      // the bucket, or storage.js's config changed since. Nothing else on
      // the request is wrong, so a clear 404 is more honest than a crash.
      return res.status(404).json({ error: 'This file could not be found in storage. It may have been created before persistent storage was configured — try re-uploading the document.' });
    }
