...
- File storage — `services/storage.js` persists every uploaded PDF, profile photo, and signature/stamp PNG to Supabase Storage (a private bucket, served back out through the app's own authenticated routes rather than a public URL) whenever `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` are set, which is required for correct behavior on Vercel — local/`/tmp` disk there is either read-only or wiped between requests. Falls back to local disk only when those aren't set, for simple local dev.
